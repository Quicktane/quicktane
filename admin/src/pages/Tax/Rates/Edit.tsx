...
import type { TaxRate, TaxZone } from "@/types/tax";
...
export function TaxRatesEdit() {
  ...
  const loadData = useCallback(async () => {
    try {
      const [rateResponse, zonesResponse] = await Promise.all([
        api.get<{ data: TaxRate }>(`/admin/tax/rates/${uuid}`),
        api.get<{ data: TaxZone[] }>("/admin/tax/zones"),
      ]);

      const taxRate = rateResponse.data.data;
      setTaxZones(zonesResponse.data.data);

      form.reset({
        name: taxRate.name,
        tax_zone_id: taxRate.tax_zone_id,
        rate: taxRate.rate,
  ...
  const handleSubmit = async (values: TaxRateFormValues) => {
    ...
    try {
      await api.put(`/admin/tax/rates/${uuid}`, values);
