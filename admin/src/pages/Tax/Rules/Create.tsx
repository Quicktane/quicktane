...
import type { TaxRate, TaxClass } from "@/types/tax";
...
export function TaxRulesCreate() {
  ...
  const loadOptions = useCallback(async () => {
    try {
      const [ratesResponse, classesResponse] = await Promise.all([
  ...
  const handleSubmit = async (values: TaxRuleFormValues) => {
    ...
    try {
      await api.post("/admin/tax/rules", values);
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        description="Link a tax rate to product and customer tax classes"
      />
