...
const productSchema = z.object({
  ...
  attribute_set_uuid: z.string().min(1, "Attribute set is required"),
...
export function ProductsCreate() {
  ...
  const [attributeSetsLoading, setAttributeSetsLoading] = useState(true);
  const [setAttributes, setSetAttributes] = useState<
    AttributeSetAttribute[]
  >([]);
  const [setAttributesLoading, setSetAttributesLoading] = useState(false);
  ...
  const watchedAttributeSetUuid = form.watch("attribute_set_uuid");
  ...
  const loadSetAttributes = useCallback(async (setUuid: string) => {
    if (!setUuid) {
      setSetAttributes([]);
      return;
    }

    setSetAttributesLoading(true);

    try {
      const response = await api.get<{ data: AttributeSet }>(
        `/admin/catalog/attribute-sets/${setUuid}`,
      );
      ...
      toast.error("Failed to load attribute set details");
    } finally {
      setSetAttributesLoading(false);
    }
  }, []);

  useEffect(() => {
    if (watchedAttributeSetUuid) {
      loadSetAttributes(watchedAttributeSetUuid);
      setAttributeValues({});
    }
  }, [watchedAttributeSetUuid, loadSetAttributes]);
            ...
            <TabsContent value="attributes">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    {watchedAttributeSetUuid
                      ? "Set attribute values for this product"
                      : "Select an attribute set in the General tab first"}
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  {!watchedAttributeSetUuid ? (
                    <p className="py-4 text-center text-sm text-muted-foreground">
                      Please select an attribute set to see available
                      attributes.
                    </p>
                    ...
                    <p className="py-4 text-center text-sm text-muted-foreground">
                      No attributes in this attribute set.
                    </p>
                  ) : (
