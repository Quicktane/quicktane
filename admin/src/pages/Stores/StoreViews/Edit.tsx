...
import type { Store, StoreView } from "@/types/store";
...
export function StoreViewsEdit() {
  ...
  const loadStoreView = useCallback(async () => {
    try {
      const response = await api.get<{ data: StoreView }>(
        `/admin/store/store-views/${uuid}`,
      );
      const storeView = response.data.data;
      ...
      form.reset({
        ...
        sort_order: storeView.sort_order,
        is_active: storeView.is_active,
      });
    } catch {
      toast.error("Failed to load store view");
  ...
  const handleSubmit = async (values: StoreViewFormValues) => {
    ...
    try {
      await api.put(`/admin/store/store-views/${uuid}`, {
        ...values,
      ...
      toast.error("Failed to update store view");
    } finally {
  ...
  return (
    <div className="space-y-4">
      <PageHeader
        ...
        description="Update store view details"
      />
